...
import type { ShotPhase, Strategy, Trade } from '../lib/types'
...
export default function TradeDetail({ trade, strategy, onBack, onEdit, onDelete }: Props) {
  ...
  return (
    <div className="mx-auto min-h-screen w-full max-w-4xl px-5 py-10">
            ...
            {trade.rMultiple !== null && (
              <span
                className={`rounded-md px-2 py-1 font-display text-sm font-bold tabular-nums ${
                  trade.rMultiple > 0 ? 'bg-win/10 text-win' : trade.rMultiple < 0 ? 'bg-loss/10 text-loss' : 'bg-even/10 text-even'
                }`}
              >
      ...
      <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-2">
        <Panel label="Checklist">
          <ul className="space-y-2.5">
            {CHECKLIST_ITEMS.map((c) => {
              const on = trade.checklist[c.key]
              return (
                <li key={c.key} className="flex items-center gap-3">
                  <span
                    className={`flex h-5 w-5 items-center justify-center rounded-md ${
                      on ? 'text-base' : 'bg-edge text-mist/50'
                    }`}
                    style={on ? { background: color } : undefined}
                  >
                    {on ? <Check size={13} strokeWidth={3} /> : <X size={12} strokeWidth={3} />}
                  </span>
                  <span className={`text-sm font-medium ${on ? 'text-bone' : 'text-mist/60 line-through'}`}>
                    {c.label}
                  </span>
                </li>
              )
      ...
      <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
        {PHASES.map((p) => (
          <Shot
            key={p.key}
            label={p.label}
            blob={trade.screenshots[p.key]}
            onOpen={() => setLightbox(p.key)}
            accent={color}
          />
        ))}
      </div>
      ...
      {url ? (
        <button
          ...
          onMouseEnter={(e) => {
            ...
            e.currentTarget.style.boxShadow = `0 12px 32px -12px ${accent}59`
          }}
          onMouseLeave={(e) => {
