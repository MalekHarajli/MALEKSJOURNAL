...
interface Props {
  ...
  existing?: Trade
  onCancel: () => void
  onSave: (trade: Trade, pending: Partial<Record<ShotPhase, File>>) => void | Promise<void>
}
...
export default function TradeForm({ strategy, confluences, existing, onCancel, onSave }: Props) {
  ...
  // Each slot holds either a saved storage path or a File just picked.
  const [shots, setShots] = useState<Partial<Record<ShotPhase, File | string>>>({
  ...
  const save = async () => {
    ...
    // Split the slots: Files are new uploads, strings are already stored.
    const pending: Partial<Record<ShotPhase, File>> = {}
    const keep: Partial<Record<ShotPhase, string>> = {}
    for (const phase of PHASES) {
      const slot = shots[phase.key]
      if (!slot) continue
      if (typeof slot === 'string') keep[phase.key] = slot
      else pending[phase.key] = slot
    }
  ...
  return (
    <div className="mx-auto min-h-screen w-full max-w-3xl px-5 py-10">
      ...
      <div className="mt-8 flex items-center justify-between gap-3 border-t border-edge pt-6">
        <p className={`text-xs ${error ? 'text-loss' : 'text-mist/60'}`}>
          {error ?? (valid ? '' : 'Pick a result to save.')}
        </p>
