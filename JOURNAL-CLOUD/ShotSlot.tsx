...
interface Props {
  label: string
  hint: string
  /** A File the user just picked, or a storage path already saved. */
  value: File | string | undefined
  onChange: (file: File | undefined) => void
}

export default function ShotSlot({ label, hint, value, onChange }: Props) {
  const inputRef = useRef<HTMLInputElement>(null)
  const [dragOver, setDragOver] = useState(false)
  const url = useShotUrl(value)
  ...
  return (
    <div>
      ...
      {url ? (
        ...
        >
          <ImagePlus size={20} />
          <span className="text-xs">{value ? 'Loading…' : hint}</span>
        </button>
      )}
    </div>
  )
