...
import { useShotUrl } from './useObjectUrl'
  ...
  return (
    <div className="mx-auto min-h-screen w-full max-w-4xl px-5 py-10">
      ...
      <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
        {PHASES.map((p) => (
          <Shot key={p.key} label={p.label} path={trade.screenshots[p.key]} onOpen={() => setLightbox(p.key)} />
        ))}
      </div>

      {lightbox && trade.screenshots[lightbox] && (
        <Lightbox
          path={trade.screenshots[lightbox]!}
...
function Shot({ label, path, onOpen }: { label: string; path: string | undefined; onOpen: () => void }) {
  const url = useShotUrl(path)
  return (
    <div>
...
function Lightbox({ path, label, onClose }: { path: string; label: string; onClose: () => void }) {
  const url = useShotUrl(path)
  useEffect(() => {
