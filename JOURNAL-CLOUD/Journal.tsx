...
import { ArrowLeft, Camera, Check, ChevronDown, LogOut, Pencil, Plus, Settings2, ShieldCheck, Trash2, X } from 'lucide-react'
      ...
      {open && (
        <div className="mt-4 rounded-2xl border border-edge bg-card p-5 animate-view-in">
          <p className="text-sm text-mist">
            Signed in as <span className="font-semibold text-bone">{email}</span>
          </p>
          <p className="mt-2 text-sm leading-relaxed text-mist/80">
            Your trades are saved to your account, not this browser. Sign in anywhere and they
            will be there.
          </p>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3">
            <button
              onClick={onSignOut}
              className="inline-flex items-center gap-2 rounded-xl border border-edge px-4 py-2 text-sm font-semibold
                text-mist transition-colors hover:border-edge-lit hover:text-bone"
            >
              <LogOut size={14} />
              Sign out
            </button>
            ...
            >
              <Trash2 size={14} />
              {confirmClear ? 'Tap again to delete everything' : `Delete all ${tradeCount} trades`}
            </button>
          </div>
