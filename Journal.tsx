...
import type { Confluence, Strategy, Trade } from '../lib/types'
import { computeStats, confluenceBreakdown } from '../lib/stats'
import { fmtDateShort, fmtR } from '../lib/format'
import { ACCENT, GRADE_CHIP, OUTCOME_META } from '../lib/colors'
      ...
      {trades.length === 0 ? (
        ...
        <>
          <div className="overflow-hidden rounded-2xl border border-edge bg-card">
            <div className="hidden grid-cols-[86px_52px_60px_100px_56px_70px_1fr_60px] gap-3 border-b border-edge px-5 py-3 text-[11px] font-semibold uppercase tracking-[0.14em] text-mist/70 sm:grid">
              <span>Date</span>
              <span>Sym</span>
              <span>Bias</span>
              <span>Outcome</span>
                  ...
                  <li key={t.id} className="border-b border-edge/60 last:border-b-0">
                    <button
                      ...
                      className="grid w-full grid-cols-2 items-center gap-3 px-5 py-3.5 text-left text-sm transition-colors
                        hover:bg-raise sm:grid-cols-[86px_52px_60px_100px_56px_70px_1fr_60px]"
                    >
